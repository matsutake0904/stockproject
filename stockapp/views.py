...
from stockapp.myPython_package import utils, scraype, my_lstm
...
def topfunc(request):
    ...
    logging.debug('STOCK_LIST {}'.format(object_list))
    close=0
    error0=""
    num = 6502
    stock_list=[]
    ...
    ##Scrayping And AI request
        ...
        if StockExist.objects.filter(stock_id =num).count() == 0:
            ...
            if req:
                ...
                try:
                    ...
                    close=0
                    error0 = e
                    return render(request, 'top.html', {'error':error0}) 
        ...
        boo, history = model.train_model(data_set[3,:])
        success_ai, predicted = model.predict(data_set[3,:])
        close=predicted
        stock_num = StockExist.objects.get(stock_id=num)
        stock_list = StockData.objects.filter(stock_id = stock_num).order_by('date_data')
        logging.debug('STOCK_LIST {}'.format(stock_list))

    return render(request, 'top.html', {'objects': stock_list,'num': num, 'close': close, 'error':error0})    
    # return render(request, 'chart.html', {'objects': object_list, 'close': close, 'error':error0})    
